...
static void TestContextSwitching() {
  ...
  trapframe_t tf = {.rip = code_addr.vaddr_value, .rsp = stack_addr.vaddr_value};
  memcpy((trapframe_t *)(p2_stack_bottom + KERNEL_STACK_SIZE -
                         sizeof(trapframe_t)), &tf, sizeof(trapframe_t));
  memcpy((trapframe_t *)(p3_stack_bottom + KERNEL_STACK_SIZE -
                         sizeof(trapframe_t)), &tf, sizeof(trapframe_t));

  // create user frames for p2, p3
  // create contexts for p2, p3
  // actually user frames & contexts does not matter at the starting point
  p2.context.regs.rsp = p2_stack_bottom + KERNEL_STACK_SIZE -
                        sizeof(trapframe_t) - sizeof(userframe_t);
  p3.context.regs.rsp = p3_stack_bottom + KERNEL_STACK_SIZE -
                        sizeof(trapframe_t) - sizeof(userframe_t);
