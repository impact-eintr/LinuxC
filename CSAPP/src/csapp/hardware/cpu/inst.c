...
// DFA to parse instruction(指令) in one-time left-right scanning
static inst_parser_t *parse_instruction_next(inst_parser_t *p, char c) {
  assert(p != NULL);
  ...
  case INST_PARSE_SPACE_SRC_OPERAND:
    if (c == '$' || c == '%' || c == '(' ||
      ...
      return parse_operand_next(p, c); // parse the operand
    } else if (c == ' ' || c == '\t' || c == '\r') {
      return p;
    } else if (c == '\n') {
      p->inst_state = INST_PARSE_PARSED;
  ...
  case INST_PARSE_DST_OPERAND:
    ...
    return p;
  case INST_PARSE_PARSED:
    assert(0);
  }
}

// DFA to parse operand(操作数) in one-time left-right scanning
static inst_parser_t *parse_operand_next(inst_parser_t *p, char c) {
  assert(p != NULL);
...
static inst_parser_t *parse_effective_address_next(inst_parser_t *p, char c) {
  ...
  case MEM_PARSE_LEFT_PARENTHESIS: // *(...
    ...
    assert(0);
  case MEM_PARSE_FIRST_REGISTER: // *(reg1...
    if ('a' <= c && c <= 'z') {
      // parsing reg1
      p->trie_node = trie_next(p->trie_node, c);
      assert(p->trie_node != NULL);
    ...
    assert(0);
  case MEM_PARSE_SECOND_REGISTER: // *(*,reg2...
    ...
    assert(0);
  case MEM_PARSE_SCALE: // *(*,reg2,scale...
    if (c == '1' || c == '2' || c == '4' || c == '8') {
      p->scal = c - '0';
      p->mem_state = MEM_PARSE_SCALE_PARSED;
    ...
    assert(0);
  case MEM_PARSE_SCALE_PARSED: // *(*,reg2,[1,2,4,8]
    if (c == ')') {
      p->mem_state = MEM_PARSE_RIGHT_PARENTHESIS;
      return p;
    }
