...
static uint64_t get_prevfree(uint64_t header_vaddr) {
  return get_field32_block_ptr(header_vaddr, MIN_EXPLICT_FREE_LIST_BLOCKSIZE, 4);
}

static uint64_t get_nextfree(uint64_t header_vaddr) {
  return get_field32_block_ptr(header_vaddr, MIN_EXPLICT_FREE_LIST_BLOCKSIZE, 8);
}

static int set_prevfree(uint64_t header_vaddr, uint64_t prev_vaddr) {
  set_field32_block_ptr(header_vaddr, prev_vaddr, MIN_EXPLICT_FREE_LIST_BLOCKSIZE, 4);
  return 1;
}

static int set_nextfree(uint64_t header_vaddr, uint64_t next_vaddr) {
  set_field32_block_ptr(header_vaddr, next_vaddr, MIN_EXPLICT_FREE_LIST_BLOCKSIZE, 8);
  return 1;
}
...
static int update_head(linkedlist_internal_t *this, uint64_t block_vaddr) {
  ...
  assert(block_vaddr == NULL_ID || get_blocksize(block_vaddr) >= MIN_EXPLICT_FREE_LIST_BLOCKSIZE);
...
int explicit_list_initialize_free_block() {
  ...
  // initialize explicit list
  explicit_list_init();
  explicit_list_insert(first_header);
  // initallize small block list
  small_list_init();
  return 1;
}
...
uint64_t explicit_list_search_free_block(uint32_t payload_size, uint32_t *alloc_blocksize) {
  ...
  if (payload_size <= 4) {
    ...
    *alloc_blocksize = round_up(payload_size, 8) + 4 + 4; // prev_ptr + next_ptr
    assert((*alloc_blocksize) >= MIN_EXPLICT_FREE_LIST_BLOCKSIZE);
  }
...
void explicit_list_check_free_block() {
  ...
  check_size_list_correctness(&explicit_list, &i_free_block, MIN_EXPLICT_FREE_LIST_BLOCKSIZE, 0xFFFFFFFF);
}
