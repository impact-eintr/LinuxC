...
extern linkedlist_internal_t explicit_list;
...
static int is_null_node(uint64_t header_vaddr) {
  if (get_firstblock() <= header_vaddr && header_vaddr <= get_lastblock() &&
      header_vaddr % 8 == 4) {
    return 0;
  }
  return 1;
...
static int set_redblack_tree_parent(uint64_t header_vaddr, uint64_t parent_vaddr) {
  set_field32_block_ptr(header_vaddr, parent_vaddr,  MIN_REDBLACK_TREE_BLOCKSIZE, 4);
  return 1;
}
...
static int set_redblack_tree_color(uint64_t header_vaddr, rb_color_t color) {
  ...
  uint64_t footer_vaddr = get_footer(header_vaddr);
  *(uint32_t *)&heap[footer_vaddr] &= 0xFFFFFFFD; // FFFFFFF1101
...
static int set_redblack_tree_key(uint64_t header_vaddr, uint64_t blocksize) {
  ...
  set_blocksize(header_vaddr, blocksize);
  return 1;
}
...
static void redblack_tree_print() { bst_internal_print(rbt.root, &i_node); }
...
int redblack_tree_initialize_free_block() {
  ...
  // init list for samll block size == 16
  explicit_list_init();
  // init list for samll block size == 8
  small_list_init();
